...
class AoC2024_day21 : public AoC {
  ...
  private:
	...
	std::set<std::string> translate(std::map<char, std::map<char, std::set<std::string>>>& steps, const std::string code);
	size_t get_shortest(std::set<std::string>& codes);
...
size_t AoC2024_day21::get_shortest(std::set<std::string>& codes) {
...
std::set<std::string> AoC2024_day21::translate(std::map<char, std::map<char, std::set<std::string>>>& steps, const std::string code) {
	...
	for (size_t i = 0; i < code.size(); i++) {
		...
		if (steps.count(start)) {
			if (steps[start].count(code[i])) {
				...
				if (result.empty()) {
					result = steps[start][code[i]];
				} else {
					tmp.swap(result);

					for (auto& step : steps[start][code[i]]) {
						for (auto& subres : tmp) {
							result.emplace(subres + step);
						}
					}
				}
...
std::map<char, std::map<char, size_t>> AoC2024_day21::get_steps_sizes_map(const size_t repeats, std::map<char, std::map<char, std::set<std::string>>>& steps) {
	...
	for (auto& start : steps) {
		for (auto& end : start.second) {
			subresult = end.second;
			for (size_t r = 1; r < repeats; r++) {
				subresult = translate(steps_arrow_, subresult);
			}
			size = get_shortest(subresult);
...
int64_t AoC2024_day21::get_code_complexities_sum(const bool part2) {
	...
	if (part2) {
		arrow_sizes_ = get_steps_sizes_map(25, steps_arrow_);
	} else {
		prepare_steps_maps();
		arrow_sizes_ = get_steps_sizes_map(2, steps_arrow_);
	}

	for (uint32_t i = 0; i < codes_.size(); i++) {
		subresult = {codes_[i]};
		subresult = translate(steps_numeric_, subresult);
		get_shortest(subresult);

		size = INT64_MAX;

		for (auto& code : subresult) {
			start = 'A';
			x = 0;
